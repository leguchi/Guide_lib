...
if (context.params.event.data.values[0] === 'step9CodeExamplesBan1') {
        ...
        "components": [
          {
            "style": 4,
            "label": `Back`,
            "custom_id": `backStep9CodeExamplesBan`,
            "disabled": false,
            "type": 2
          }
        ]
    ...
    "embeds": [
      {
        ...
        "description": `**It doesn't say which trigger to use.**\n\nIt doesn't, but the first image shows the slash command builder, and is also mentioned, so it hints how this is a \`command\` endpoint.`,
        "color": 0x00ff00,
        "url": `https://autocode.com/guides/how-to-build-a-discord-bot/#ban-kick-mute`
      }
    ]
  });
} else if (context.params.event.data.values[0] === 'step9CodeExamplesBan2') {
  await lib.discord.channels['@0.3.0'].messages.update({
    message_id: context.params.event.message.id,
    "channel_id": `${context.params.event.channel_id}`,
    "content": "",
    ...
    "embeds": [
      {
        "type": "rich",
        "title": `Step 9: Code examples for bot actions`,
        "description": `**I'm not sure what the third command should be.**\n\nI'm just assuming they meant \`/mute\`, from the anchor-link. If you'd like to create your own \`/mute\` command, you definitely can to make it the third :joy:.`,
        "color": 0x00ff00,
        "url": `https://autocode.com/guides/how-to-build-a-discord-bot/#ban-kick-mute`
      }
    ]
  });
} else if (context.params.event.data.values[0] === 'step9CodeExamplesBan3') {
  await lib.discord.channels['@0.3.0'].messages.update({
    message_id: context.params.event.message.id,
    "channel_id": `${context.params.event.channel_id}`,
    "content": "",
...
} else if (context.params.event.data.values[0] === 'step9CodeExamplesBan4') {
  await lib.discord.channels['@0.3.0'].messages.update({
    message_id: context.params.event.message.id,
    "channel_id": `${context.params.event.channel_id}`,
    "content": "",
