...
if (context.params.event.data.values[0] === 'step9CodeExamplesRestrictPermissions1') {
        ...
        "components": [
          {
            "style": 4,
            "label": `Back`,
            "custom_id": `backStep9CodeExamplesRestrictPermissions`,
            "disabled": false,
            "type": 2
          }
        ]
...
} else if (context.params.event.data.values[0] === 'step9CodeExamplesRestrictPermissions2') {
  await lib.discord.channels['@0.3.0'].messages.update({
    message_id: context.params.event.message.id,
    "channel_id": `${context.params.event.channel_id}`,
    "content": "",
    ...
    "embeds": [
      {
        "type": "rich",
        "title": `Step 9: Code examples for bot actions`,
        "description": `**I'm not sure what permissions are.**\n\nPermissions are what are given to each user as a member of a guild. They include things such as being able to send invites, writing messages, audit log visibility, etc. In some cases, permissions are given through roles, therefor everyone who has that role has up to those permissions (unless they have other roles). In other cases, permissions can be given individually. For more info, check [here](https://discord.com/developers/docs/topics/permissions#permissions).`,
        "color": 0x00ff00,
        "url": `https://autocode.com/guides/how-to-build-a-discord-bot/#restrict-commands-permissions`
      }
    ]
  });
} else if (context.params.event.data.values[0] === 'step9CodeExamplesRestrictPermissions3') {
  await lib.discord.channels['@0.3.0'].messages.update({
    message_id: context.params.event.message.id,
    "channel_id": `${context.params.event.channel_id}`,
    "content": "",
...
} else if (context.params.event.data.values[0] === 'step9CodeExamplesRestrictPermissions4') {
  await lib.discord.channels['@0.3.0'].messages.update({
    message_id: context.params.event.message.id,
    "channel_id": `${context.params.event.channel_id}`,
    "content": "",
